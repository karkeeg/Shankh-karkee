...
const Onboarding3 = () => {
    ...
    return (
      <motion.div className='flex flex-col space-y-[15px] sm:space-y-[20px] lg:space-y-[25px] justify-center items-center text-center p-4 sm:p-6 lg:p-8'>
        <div className='w-full max-w-[777px] items-center pt-[20px] sm:pt-[25px] lg:pt-[29px] pl-[15px] sm:pl-[20px] lg:pl-[24px] pr-[15px] sm:pr-[20px] lg:pr-[29px] pb-[20px] sm:pb-[22px] lg:pb-[24px] rounded-2xl shadow-md space-y-[10px] sm:space-y-[12px] lg:space-y-[14px] flex flex-col bg-white min-h-[380px] sm:h-[420px] lg:h-[456px]'>
          <h1 style={{fontFamily : "Poppins"}} className='text-[#34856C] leading-tight sm:leading-8 lg:leading-10 text-[24px] sm:text-[28px] lg:text-[32px] font-semibold'>What do you need from the app ?</h1>
          <p style={{fontFamily : "Inter"}} className='text-[14px] sm:text-[16px] lg:text-[18px] w-full max-w-[631px] px-2 sm:px-4 lg:px-0'>Now let us delve into which skill do you want to enhance?</p>
          <div style={{fontFamily : "Inter"}} className='grid grid-cols-1 sm:grid-cols-2 gap-[12px] sm:gap-[14px] lg:gap-[16px] w-full'> 
            {skillSet.map((item, index) => {
                return (
                  <button 
                    onClick={() => setIsActive2((prev) => ({...prev, [item] : !prev[item]}))}  
                    className={`flex flex-col border-[1px] border-[#D9E0E6] p-[10px] sm:p-[11px] lg:p-[12px] w-full max-w-[320px] h-[70px] sm:h-[80px] lg:h-[85px] ${isActive2[item] ? "bg-[#EFFDFA]" : null} space-y-[8px] sm:space-y-[9px] lg:space-y-[10px] items-center transition-colors hover:bg-gray-50`}
                  >
