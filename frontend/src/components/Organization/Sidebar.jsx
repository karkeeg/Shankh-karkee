...
const Sidebar = ({status, setStatus, setSidebarOpen}) => {
  ...
  return (
    <div style={{fontFamily : "Poppins"}} className='font-semibold w-[280px] sm:w-[305px] flex justify-between flex-col pr-[22.52px] pb-[20.03px] pl-[22.52px] pt-[20.03px] text-white bg-[#34856C] h-screen'>
        <div className='flex space-y-[50px] flex-col'>
            {/* Header with Logo and Close Button */}
            <div className="flex justify-between items-center">
                <img className='w-[141px] h-[38px]' src={Logo2} alt="Logo"></img>
            ...
            <button onClick={() => handleNavigation("home")} className={`flex rounded-lg space-x-2 p-[7.51px] items-center ${status == "home" ? "bg-[white] text-black " : null}`}>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-house-icon lucide-house"><path d="M15 21v-8a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v8"/><path d="M3 10a2 2 0 0 1 .709-1.528l7-5.999a2 2 0 0 1 2.582 0l7 5.999A2 2 0 0 1 21 10v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>
                <span>Dashboard</span>
            </button>
            <button onClick={() => handleNavigation("overview")} className={`flex rounded-lg space-x-2 p-[7.51px] items-center ${status == "overview" || status == "addUser" || status == "editUser" ? "bg-[white] text-black " : null}`}>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="white" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-users-icon lucide-users"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                <span>User Overview</span>
            </button>
            <button onClick={() => handleNavigation("profile")} className={`flex rounded-lg space-x-2 p-[7.51px] items-center ${status == "profile" ? "bg-[white] text-black " : null}`}>
