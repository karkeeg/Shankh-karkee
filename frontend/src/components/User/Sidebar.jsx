...
const Sidebar = ({ status, setStatus, setSidebarOpen }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      // Close sidebar when switching from mobile to desktop if it's open
    ...
    <>
      ...
      {isMobile && (
        <button
          ...
          className="lg:hidden fixed top-4 left-4 z-40 p-2 rounded-md bg-[#34856C] text-white"
        >
          <svg
      ...
      {isMobile && (
        <div
          className={`fixed inset-0 bg-black bg-opacity-50 z-30 transition-opacity duration-300 ${
            setSidebarOpen ? "opacity-100" : "opacity-0 pointer-events-none"
          }`}
      ...
      <div
        ...
        className={`font-semibold w-[280px] sm:w-[305px] flex justify-between flex-col pr-[22.52px] pb-[20.03px] pl-[22.52px] pt-[20.03px] text-white bg-[#34856C] h-screen fixed lg:relative z-40 transition-transform duration-300 ${
          isMobile
            ? setSidebarOpen
              ? "translate-x-0"
              : "-translate-x-full"
